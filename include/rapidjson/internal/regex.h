...
*/
		...
		public:
			...
			GenericRegex(const Ch *source,
			             Allocator *allocator = 0) : ownAllocator_(allocator ? 0 : RAPIDJSON_NEW(Allocator)()),
		...
		private:
			enum Operator {
				kZeroOrOne, kZeroOrMore, kOneOrMore, kConcatenation, kAlternation, kLeftParenthesis
			};
			...
			struct Frag {
				Frag(SizeType s,
				     SizeType o,
				     SizeType m) : start(s), out(o), minIndex(m) {}
			...
			void
			PushOperand(Stack<Allocator> &operandStack,
			            unsigned codepoint) {
			...
			void
			ImplicitConcatenation(Stack<Allocator> &atomCountStack,
			...
			bool
			Eval(Stack<Allocator> &operandStack,
			     Operator op) {
				switch (op) {
					case kConcatenation:
			...
			bool
			EvalQuantifier(Stack<Allocator> &operandStack,
			               unsigned n,
			               unsigned m) {
			...
			template<typename InputStream>
			bool
			ParseUnsigned(DecodedStream<InputStream, Encoding> &ds,
			              unsigned *u) {
				unsigned r = 0;
			...
			template<typename InputStream>
			bool
			ParseRange(DecodedStream<InputStream, Encoding> &ds,
			           SizeType *range) {
				bool isBegin = true;
				bool negate = false;
				int step = 0;
			...
			template<typename InputStream>
			bool
			CharacterEscape(DecodedStream<InputStream, Encoding> &ds,
		...
		public:
			...
			GenericRegexSearch(const RegexType &regex,
			                   Allocator *allocator = 0) : regex_(regex), allocator_(allocator), ownAllocator_(0),
		...
		private:
			...
			template<typename InputStream>
			bool
			SearchWithAnchoring(InputStream &is,
			                    bool anchorBegin,
			                    bool anchorEnd) {
			...
			size_t
			GetStateSetSize() const {
				return (regex_.stateCount_ + 31) / 32 * 4;
			}

			// Return whether the added states is a match state
			bool
			AddState(Stack<Allocator> &l,
			         SizeType index) {
			...
			bool
			MatchRange(SizeType rangeIndex,
